...
public class CinderConeProfile implements VolcanoProfile {
    ...
    public void tickOffline(ServerLevel level, VolcanoRecord record, OfflineBudget budget) {
        ...
        if (record.getPhase() == VolcanoPhase.INCUBATING) {
            // Abstract plume rise: ~1 block per 200 ticks toward surface, capped by ops budget.
    ...
    private void advanceCinderPhaseOffline(VolcanoRecord record) {
        ...
        switch (record.getPhase()) {
            ...
            case INCUBATING -> {
                // Breach remains loaded-detail; offline only grows plume height.
            }
        }
    }

    @Override
    public void applyPendingBudgets(Level level, VolcanoCoreBlockEntity blockEntity, VolcanoRecord record) {
        VolcanoProfile.super.applyPendingBudgets(level, blockEntity, record);
        int ash = record.getPendingAshLayers();
        if (ash <= 0 || level.isClientSide) {
            return;
        }
        // Light paint on reload — not a full eruption replay.
        int ops = Math.min(ash, TephraConfig.COMMON.offlineMaxBlockOpsPerVolcano.get());
        BlockPos origin = blockEntity.getBlockPos();
        float radius = Math.max(4.0f, record.getAbstractFootprintRadius());
        for (int i = 0; i < ops; i++) {
        ...
        record.setPendingAshLayers(Math.max(0, ash - ops));
    }
